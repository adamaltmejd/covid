...
tar_option_set(
    library = .libPaths(),
    packages = c(
        "data.table", "dplyr", "fst", "readxl", "stringr", "qs",
        "Matrix", "ggplot2", "cowplot", "hrbrthemes", "wesanderson",
        "forcats", "gdtools", "tools", "curl", "RCurl", "lubridate", "callr"
    ),
...
list(
    # Run new download daily
    # FHM
    tarchetypes::tar_change(get_fhm_latest, download_latest_fhm(folder = file.path("data", "FHM")), Sys.Date()),
    tarchetypes::tar_change(f_fhm_paths, list.files(file.path("data", "FHM"), pattern = "^Folkhalso", full.names = TRUE), get_fhm_latest),
